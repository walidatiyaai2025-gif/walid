...
# SEC-P0-003: the final Browser ownership proof must remain before the physical submit.
...
# SEC-P0-001: callers and adapter use the canonical durable reservation service;
# caller-side random DispatchId production paths are forbidden.
...
$host = Require-Text 'src/PCCExecutive.App/Presentation/IntegratedPresentationGateway.cs' '_dispatchReservations = new CanonicalDispatchReservationService(store);'
...
if ($host.Contains('DispatchId.New()')) { throw 'Unsafe Manager caller DispatchId.New() remains.' }
...
# P0-004 / rollover: canonical recovery stays before AutoResume and PR34 hardened
# rollover composition is attached directly to the current production host.
...
# Browser.Acceptance duplicate member blocker must be genuinely removed.
...
# The E2E project must execute against the real Windows production host.
