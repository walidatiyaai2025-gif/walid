...
if ($version -notmatch '^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$') { throw "VERSION '$version' is not valid semantic version syntax." }
$dotnetVersion = (& dotnet --version).Trim()
if ($LASTEXITCODE -ne 0 -or $dotnetVersion -notmatch '^10\.') { throw ".NET 10 SDK is required. Detected '$dotnetVersion'." }

$solutions = @(Get-ChildItem -Path $repoRoot -Recurse -File -Filter '*.slnx' | Where-Object { $_.FullName -notmatch '[\\/](artifacts|bin|obj)[\\/]' } | Sort-Object FullName)
if ($solutions.Count -eq 0) { $solutions = @(Get-ChildItem -Path $repoRoot -Recurse -File -Filter '*.sln' | Where-Object { $_.FullName -notmatch '[\\/](artifacts|bin|obj)[\\/]' } | Sort-Object FullName) }
$srcProjects = @(Get-ChildItem -Path (Join-Path $repoRoot 'src') -Recurse -File -Filter '*.csproj' -ErrorAction SilentlyContinue | Sort-Object FullName)
...
# The canonical solution can lag parallel Worker integration. Building every src project is mandatory
# so WPF, Browser, Infrastructure, PCC/GitHub and Updater cannot disappear behind a stale solution file.
...
$testProjects = @(Get-ChildItem -Path (Join-Path $repoRoot 'tests') -Recurse -File -Filter '*.csproj' -ErrorAction SilentlyContinue | Sort-Object FullName)
if ($testProjects.Count -eq 0) {
    ...
    Write-Host 'PARTIAL_SOURCE_BUILD_VALID: source projects built; product test lane is not integrated.'
    exit 0
}
...
foreach ($testProject in $testProjects) {
    $args = @('test',$testProject.FullName,'--configuration',$Configuration,'--logger','trx','--results-directory',$resultsDir,'-p:Version='+$version,'-p:ContinuousIntegrationBuild='+[bool]$env:GITHUB_ACTIONS)
