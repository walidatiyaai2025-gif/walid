...
try{
    ...
    $expected=@{DOMAIN='PENDING_PR';APPLICATION='PENDING_PR';PCC='PENDING_PR';GITHUB='PENDING_PR';PERSISTENCE='BLOCKED';BROWSER='PENDING_PR';UI='BLOCKED';UPDATER='PENDING_PR';INSTALLER='PENDING_PR'}
    ...
    if($matrix.Overall -ne 'BLOCKED_DEPENDENCY'){throw 'Current rehearsal must remain BLOCKED_DEPENDENCY until Worker 2 and Worker 4 land.'}
