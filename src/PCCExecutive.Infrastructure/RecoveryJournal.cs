...
public sealed class RecoveryJournalService
{
    ...
    private static Task EnsureTableAsync(SqliteConnection connection, CancellationToken cancellationToken) =>
        SqliteDurabilityConnection.ExecuteAsync(
            connection,
            "CREATE TABLE IF NOT EXISTS recovery_journal(event_id TEXT NOT NULL PRIMARY KEY, project_run_id TEXT NULL, kind TEXT NOT NULL, detail TEXT NOT NULL, created_at TEXT NOT NULL);",
            cancellationToken);
}
