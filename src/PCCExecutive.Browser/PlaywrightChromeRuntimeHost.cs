...
public sealed class ChromeExecutableLocator : IChromeExecutableLocator
{
    public string LocateChrome()
    {
        ...
        return chrome ?? throw new FileNotFoundException("Google Chrome was not found. Set PCC_EXECUTIVE_CHROME_PATH to the Chrome executable.");
    }
}
...
public sealed class WindowsBrowserWindowVisibilityController : IBrowserWindowVisibilityController
{
    ...
    public Task<bool> HideAsync(int processId, CancellationToken cancellationToken = default)
    {
        ...
        return Task.FromResult(window != IntPtr.Zero && ShowWindow(window, SwHide));
    }
    ...
    private static IntPtr FindTopLevelWindow(int processId)
    {
        ...
        _ = EnumWindows((window, _) =>
        {
            ...
            if (ownerPid == processId && IsWindowVisible(window))
            {
                result = window;
                return false;
            }
...
public sealed class PlaywrightChromeRuntimeHost : IBrowserRuntimeHost, IPlaywrightPageProvider
{
    ...
    public async Task<BrowserRuntimeRecord> LaunchAsync(BrowserSessionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var profilePath = Path.Combine(_profileRoot, SanitizeRuntimeId(runtimeId));
        ...
        try
        {
            ...
            var context = browser.Contexts.FirstOrDefault() ?? throw new InvalidOperationException("Chrome CDP connection has no browser context.");
    ...
    public async Task<bool> RecoverAsync(BrowserRuntimeRecord runtime, CancellationToken cancellationToken = default)
    {
        ...
        if (context is null)
            return false;
        var page = context.Pages.FirstOrDefault() ?? await context.NewPageAsync().ConfigureAwait(false);
        _connections[runtime.RuntimeId] = new(process, browser, page, runtime.ContextIdentity ?? Guid.NewGuid().ToString("N"));
        return true;
    }
    ...
    {
        if (runtime.ProcessId is not > 0)
            throw new InvalidOperationException("Runtime has no process identity.");
    ...
    public Task KillAsync(BrowserRuntimeRecord runtime, OwnershipProof proof, CancellationToken cancellationToken = default)
    {
        ...
        {
            // Already gone is a successful terminal state for an owned runtime.
        }
    ...
    private static async Task<string> WaitForDevToolsEndpointAsync(string profilePath, Process process, CancellationToken cancellationToken)
    {
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            ...
            if (File.Exists(activePortFile))
            {
                try
                {
                    var lines = await File.ReadAllLinesAsync(activePortFile, cancellationToken).ConfigureAwait(false);
                    if (lines.Length > 0 && int.TryParse(lines[0], out var port) && port is > 0 and <= 65535)
                        return $"http://127.0.0.1:{port}";
                }
                catch (IOException)
                {
                    // Chrome may still be writing the file; retry without generating browser traffic.
                }
            }
    ...
    private static string SanitizeRuntimeId(string runtimeId)
    {
        var invalid = Path.GetInvalidFileNameChars();
        var chars = runtimeId.Select(ch => invalid.Contains(ch) ? '_' : ch).ToArray();
