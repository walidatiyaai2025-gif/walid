...
public sealed class ChromeExecutableLocator : IChromeExecutableLocator
{
    public string LocateChrome()
    {
        ...
        return candidates.FirstOrDefault(File.Exists)
            ?? throw new FileNotFoundException("Google Chrome was not found. Set PCC_EXECUTIVE_CHROME_PATH to the Chrome executable.");
    }
}
...
public sealed class WindowsBrowserWindowVisibilityController : IBrowserWindowVisibilityController
{
    ...
    public Task<bool> HideAsync(int processId, CancellationToken cancellationToken = default)
    {
        ...
        var window = FindTopLevelWindow(processId);
        if (window == IntPtr.Zero) return Task.FromResult(false);
    ...
    public Task<bool> ShowAsync(int processId, bool bringToFront, CancellationToken cancellationToken = default)
    {
        ...
        var window = FindTopLevelWindow(processId);
        if (window == IntPtr.Zero) return Task.FromResult(false);
        _ = ShowWindow(window, bringToFront ? SwRestore : SwShow);
        if (bringToFront) _ = SetForegroundWindow(window);
        return Task.FromResult(true);
    }
...
public sealed class PlaywrightChromeRuntimeHost : IBrowserRuntimeHost, IPlaywrightPageProvider
{
    ...
    private const string SourceMarkerFileName = ".pcc-source-profile";
    ...
    private static readonly HashSet<string> SkippedProfileFiles = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        FileOwnershipMarkerStore.MarkerFileName, SourceMarkerFileName
    };

    private sealed record Connection(Process Process, IBrowser Browser, IPage Page, string ContextIdentity, string ProfilePath);

    private readonly string _profileRoot;
    private readonly IChromeExecutableLocator _chromeLocator;
    private readonly IBrowserWindowVisibilityController _windows;
    ...
    public PlaywrightChromeRuntimeHost(
        string profileRoot,
        IChromeExecutableLocator? chromeLocator = null,
        IBrowserWindowVisibilityController? windows = null)
    {
        _profileRoot = Path.GetFullPath(profileRoot);
        _chromeLocator = chromeLocator ?? new ChromeExecutableLocator();
        _windows = windows ?? new WindowsBrowserWindowVisibilityController();
    }
    ...
    public async Task<BrowserRuntimeRecord> LaunchAsync(BrowserSessionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var profilePath = ResolvePersistentProfilePath(request);
        Directory.CreateDirectory(profilePath);

        var selectedProfileSource = ResolveSelectedProfileSource();
        string? launchProfileDirectory = null;
        if (selectedProfileSource is not null)
        {
            if (string.Equals(selectedProfileSource, GptDesktopProfileSource, StringComparison.Ordinal))
            {
                SeedFromGptDesktopProfile(profilePath, cancellationToken);
            }
            else
            {
                SeedFromChromeProfile(profilePath, selectedProfileSource, cancellationToken);
                launchProfileDirectory = selectedProfileSource;
            }
        }
        ...
        if (launchProfileDirectory is not null)
            startInfo.ArgumentList.Add($"--profile-directory={launchProfileDirectory}");
        ...
        var process = Process.Start(startInfo) ?? throw new InvalidOperationException("Chrome process failed to start.");
        var processIdentity = BrowserProcessIdentity.From(process);
        var ownershipNonce = Convert.ToHexString(System.Security.Cryptography.RandomNumberGenerator.GetBytes(32));

        try
        {
            var endpoint = await WaitForDevToolsEndpointAsync(profilePath, process, cancellationToken).ConfigureAwait(false);
            ...
            if (request.DefaultVisibility == BrowserVisibility.Hidden)
                _ = await _windows.HideAsync(process.Id, cancellationToken).ConfigureAwait(false);
            else
                _ = await _windows.ShowAsync(process.Id, false, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<bool> RecoverAsync(BrowserRuntimeRecord runtime, CancellationToken cancellationToken = default)
    {
        ...
        var endpoint = await WaitForDevToolsEndpointAsync(runtime.ProfilePath, process, cancellationToken).ConfigureAwait(false);
        var playwright = await GetPlaywrightAsync(cancellationToken).ConfigureAwait(false);
        var browser = await playwright.Chromium.ConnectOverCDPAsync(endpoint).ConfigureAwait(false);
        var context = browser.Contexts.FirstOrDefault();
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (visibility == BrowserVisibility.Hidden)
            _ = await _windows.HideAsync(runtime.ProcessId.Value, cancellationToken).ConfigureAwait(false);
        else
            _ = await _windows.ShowAsync(runtime.ProcessId.Value, bringToFront, cancellationToken).ConfigureAwait(false);
    }

    public Task KillAsync(BrowserRuntimeRecord runtime, OwnershipProof proof, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (!proof.IsProven || !StringComparer.Ordinal.Equals(proof.RuntimeId, runtime.RuntimeId))
            throw new InvalidOperationException("Browser process termination refused without matching positive PCC ownership proof.");
        if (runtime.ProcessId is not > 0)
            throw new InvalidOperationException("Owned runtime has no process id.");

        if (_connections.TryRemove(runtime.RuntimeId, out var connection)) connection.Process.Dispose();
        try
        {
            using var process = Process.GetProcessById(runtime.ProcessId.Value);
            if (!process.HasExited) process.Kill(entireProcessTree: true);
        }
        catch (ArgumentException)
        {
    ...
    public Task<BrowserRuntimeTelemetry> GetTelemetryAsync(BrowserRuntimeRecord runtime, CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        if (runtime.ProcessId is not > 0)
            return Task.FromResult(new BrowserRuntimeTelemetry(runtime.RuntimeId, false, 0, 0, TimeSpan.Zero, runtime.LastHeartbeatAt, true, runtime.IsArchived));
        ...
        try
        {
            ...
            if (process.HasExited)
                return Task.FromResult(new BrowserRuntimeTelemetry(runtime.RuntimeId, false, 0, 0, TimeSpan.Zero, runtime.LastHeartbeatAt, true, runtime.IsArchived));
            return Task.FromResult(new BrowserRuntimeTelemetry(
                runtime.RuntimeId,
                true,
                1,
                ...
                DateTimeOffset.UtcNow - runtime.LastActivityAt > TimeSpan.FromMinutes(5),
                runtime.IsArchived));
        }
        catch (ArgumentException)
        {
    ...
    private static string? ResolveSelectedProfileSource()
    {
        ...
        if (string.Equals(selected, GptDesktopProfileSource, StringComparison.Ordinal)) return selected;
    ...
    private static void SeedFromGptDesktopProfile(string managedRoot, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ...
        if (!Directory.Exists(sourceRoot))
            throw new InvalidOperationException("GPTDeskTop's existing ChromeProfile was not found on this Windows account.");

        if (!PrepareManagedRootForSource(managedRoot, GptDesktopProfileSource)) return;

        var localState = Path.Combine(sourceRoot, "Local State");
        CopySharedFile(localState, Path.Combine(managedRoot, "Local State"), required: File.Exists(localState));
        CopyProfileDirectory(sourceRoot, managedRoot, cancellationToken);
        File.WriteAllText(Path.Combine(managedRoot, SourceMarkerFileName), GptDesktopProfileSource);
    }

    private static void SeedFromChromeProfile(string managedRoot, string selectedProfileDirectory, CancellationToken cancellationToken)
    {
        cancellationToken.ThrowIfCancellationRequested();
        var sourceRoot = Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData),
        ...
        if (!PrepareManagedRootForSource(managedRoot, selectedProfileDirectory)) return;

        Directory.CreateDirectory(managedRoot);
        CopySharedFile(Path.Combine(sourceRoot, "Local State"), Path.Combine(managedRoot, "Local State"), required: true);
        CopyProfileDirectory(sourceProfile, Path.Combine(managedRoot, selectedProfileDirectory), cancellationToken);
        File.WriteAllText(Path.Combine(managedRoot, SourceMarkerFileName), selectedProfileDirectory);
    }

    private static bool PrepareManagedRootForSource(string managedRoot, string sourceIdentity)
    {
        Directory.CreateDirectory(managedRoot);
        var markerPath = Path.Combine(managedRoot, SourceMarkerFileName);
        if (File.Exists(markerPath))
        {
            try
            {
                var marker = File.ReadAllText(markerPath).Trim();
                if (string.Equals(marker, sourceIdentity, StringComparison.OrdinalIgnoreCase)) return false;
            }
            catch (IOException)
            {
            }
        ...
        ClearManagedProfile(managedRoot);
        Directory.CreateDirectory(managedRoot);
        return true;
    }

    private static void ClearManagedProfile(string managedRoot)
    {
        if (!Directory.Exists(managedRoot)) return;
        try
        {
            foreach (var file in Directory.EnumerateFiles(managedRoot))
                File.Delete(file);
            foreach (var directory in Directory.EnumerateDirectories(managedRoot))
                Directory.Delete(directory, recursive: true);
        }
        catch (Exception ex) when (ex is IOException or UnauthorizedAccessException)
        {
            throw new InvalidOperationException("The persistent PCC Chrome profile is currently in use. Close the PCC-owned Chrome window and retry the profile change.", ex);
        }
    }
    ...
    private static void CopyProfileDirectory(string source, string destination, CancellationToken cancellationToken)
    {
        ...
        try
        {
            files = Directory.EnumerateFiles(source).ToArray();
        }
        catch (IOException)
        {
        ...
        try
        {
            directories = Directory.EnumerateDirectories(source).ToArray();
        }
        catch (IOException)
        {
    ...
    private static void CopySharedFile(string source, string destination, bool required)
    {
        ...
        {
            if (required)
                throw new InvalidOperationException($"Required Chrome profile file is missing: {Path.GetFileName(source)}.");
            return;
        }
    ...
    {
        var file = Path.Combine(profilePath, "DevToolsActivePort");
        var deadline = DateTimeOffset.UtcNow + TimeSpan.FromSeconds(20);
        while (DateTimeOffset.UtcNow < deadline)
        {
            cancellationToken.ThrowIfCancellationRequested();
            if (process.HasExited)
                throw new InvalidOperationException($"Chrome exited before CDP became ready (exit code {process.ExitCode}).");
            if (File.Exists(file))
            {
                try
                {
