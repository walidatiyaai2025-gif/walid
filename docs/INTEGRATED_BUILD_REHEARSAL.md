...
The current canonical branch contains Domain, Application, PCC, GitHub, Browser, WPF App, Infrastructure, Updater and Installer content. SQLite schema target is explicitly `1`. The rehearsal therefore attempts the real self-contained Setup EXE, fresh install/launch, standalone persistence smoke and default uninstall preservation. First-run is deliberately stricter than launch smoke: the installed application must actually initialize the default durable SQLite database; an unbound WPF service seam is reported as `FAIL`, never as PASS.
