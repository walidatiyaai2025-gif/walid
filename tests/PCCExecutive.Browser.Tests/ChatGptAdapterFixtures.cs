...
public static class ChatGptAdapterFixtures
{
    ...
    {
        HealthyIdle, Generating, ResponseComplete, SlowGeneration, SendingTooFast, TemporaryError,
...
{
    private const string Version = "fixture-semantic-v2";
    ...
    public ChatGptSemanticSnapshot Inspect(ChatGptHtmlFixture fixture)
    {
        ...
        var generation = stop ? D(GenerationState.Generating, .98, "fixture:stop-visible") : assistant > 0 && responseAction ? D(GenerationState.Complete, .94, "fixture:response-actions") : assistant == 0 && composer ? D(GenerationState.Idle, .90, "fixture:idle") : D(GenerationState.Unknown, .30, "fixture:generation-unknown");
        var completeness = partial || contextLimit ? ResponseCompleteness.Partial : generation.State == GenerationState.Complete && responseAction ? ResponseCompleteness.Complete : assistant == 0 ? ResponseCompleteness.None : ResponseCompleteness.Unknown;
