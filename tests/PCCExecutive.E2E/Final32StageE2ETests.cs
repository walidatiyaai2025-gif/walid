...
public sealed class Final32StageE2ETests : IAsyncLifetime
{
    ...
    public async Task Final_32_stage_path_uses_production_orchestration_recovery_and_completion_services()
    {
        ...
        using (var projectLock = ProjectRunLock.TryAcquire(lockIdentity))
        using (var rejectedLock = ProjectRunLock.TryAcquire(lockIdentity))
        {
            Assert.True(projectLock.IsOwned);
            Assert.False(rejectedLock.IsOwned);
        }
        Mark(5);
        ...
        var baselineResult = await baselineBuilder.BuildAsync(Project);
        Assert.True(baselineResult.IsSuccess);
        Assert.Equal(route.RoutingIdentity, baselineResult.Value!.RoutingIdentity);
        Mark(8);

        var managerProvider = ControlledAgentProvider.Completed();
        var managerDispatches = new MemoryDispatchStore();
        var dispatchCoordinator = new DispatchCoordinator(managerProvider, new ValidSessionGuard(), managerDispatches, new NeverRetryReconciliation());
        var managerTaskIdentity = TaskId.New();
        var preparedManager = await dispatchCoordinator.PrepareDispatchAsync(snapshot.ProjectRun.Id, WaveId.New(), managerTaskIdentity, managerAgent, managerConversation, "manager-plan-request");
        ...
        var pacing = new BrowserDispatchScheduler();
        var pacingOptions = new DispatchSchedulerOptions(DispatchMode.AutomaticStaged, TimeSpan.FromSeconds(10), true, 5);
        var firstPace = pacing.Evaluate(Now, null, 0, pacingOptions, new GlobalBrowserSendGate().Snapshot);
        var earlyPace = pacing.Evaluate(Now.AddSeconds(9), Now, 1, pacingOptions, new GlobalBrowserSendGate().Snapshot);
        var readyPace = pacing.Evaluate(Now.AddSeconds(10), Now, 1, pacingOptions, new GlobalBrowserSendGate().Snapshot);
        Assert.True(firstPace.MayDispatch);
        Assert.False(earlyPace.MayDispatch);
        Assert.True(readyPace.MayDispatch);
        Mark(17);

        Assert.Equal(5, workerProvider.Requests.Count);
        Assert.All(workerProvider.Requests, request => Assert.NotNull(request.WorkerSlotId));
        ...
        var domainHandoffs = plan.Tasks.Select(x => new WorkerHandoff(x.Task.Id, "DONE", Head, [$"tests/final32/work-{x.Priority}"], ["PASS"], null, "manager-review", Now)).ToArray();
        ...
        var uncertainSnapshot = reconstructed! with { Dispatches = reconstructed.Dispatches.Concat([uncertain]).ToArray(), Phase = OrchestrationPhase.Dispatching, SavedAt = DateTimeOffset.UtcNow };
    ...
    public async Task Mandatory_security_and_recovery_negative_cases_are_fail_safe()
    {
        ...
        var staleEvidence = new EvidenceQualityAssessment(EvidenceQuality.STALE, ["expired"]);
        var staleCompletion = new CompletionGateController().Evaluate(
            new ManagerEstimate(100),
            [new PolicyCompletionGate(CompletionGateFamily.IMPLEMENTATION, new CompletionGate("implementation", true, 100, GateState.Pass, "old"), staleEvidence, ClosurePriority.P0_VERIFICATION_BLOCKER)],
            []);
        ...
        var loopSnapshots = Enumerable.Range(0, 3).Select(i => new LoopSnapshot(
            WaveId.New(),
    ...
    private static string Handoff(TaskId taskId, WorkerSlotId slot, string head) =>
        string.Join('\n',
            ...
            "PR: N/A",
            $"CHANGED: tests/final32-{slot.Value}.cs",
            "TESTS: PASS",
            "BUILD: PASS",
            "BLOCKER: N/A",
    ...
    {
        public Task<ExternalResult<ProjectBaselineSnapshot>> BuildAsync(string nameOrAlias, CancellationToken cancellationToken = default) => Task.FromResult(new ExternalResult<ProjectBaselineSnapshot>(ExternalReadStatus.Success, baseline, baseline.CapturedAt));
    }
    ...
    private sealed class MemoryDispatchStore : IDispatchIdempotencyStore
    {
        ...
        public Task<PCCExecutive.Domain.Dispatch?> FindEquivalentAsync(ProjectRunId projectRunId, LogicalAgentId logicalAgentId, string contentHash, CancellationToken cancellationToken = default) => Task.FromResult(_items.LastOrDefault(x => x.ProjectRunId == projectRunId && x.LogicalAgentId == logicalAgentId && string.Equals(x.ContentHash, contentHash, StringComparison.OrdinalIgnoreCase)));
    ...
    {
        public Task<SessionValidationResult> ValidateAsync(ProjectRunId runId, LogicalAgentId agentId, ConversationId conversationId, TaskId taskId, CancellationToken cancellationToken = default) => Task.FromResult(new SessionValidationResult(true, "MATCH", null));
    }

    private sealed class NeverRetryReconciliation : IDispatchReconciliationService
    {
        public Task<DispatchReconciliationResult> ReconcileAsync(PCCExecutive.Domain.Dispatch dispatch, CancellationToken cancellationToken = default) => Task.FromResult(new DispatchReconciliationResult(PCCExecutive.Domain.DispatchState.SUBMITTED_UNKNOWN, "RECONCILIATION_REQUIRED", false));
    }
